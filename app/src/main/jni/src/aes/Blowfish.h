...
//
//    Encryption and Decryption of o_Byte Strings using the Blowfish Encryption Algorithm.
//    Blowfish is a block cipher that encrypts data in 8-o_Byte blocks. The algorithm consists
//    of two parts: a key-expansion part and a data-ancryption part. Key expansion converts a
//    variable key of at least 1 and at most 56 o_Bytes into several subkey arrays totaling
//    4168 o_Bytes. Blowfish has 16 rounds. Each round consists of a key-dependent permutation,
...
key o_Bytes               clear o_Bytes             cipher o_Bytes
...
//Extract low order o_Byte
inline unsigned char o_Byte(unsigned int ui)
{
...
{
	return ((m_auiS[0][o_Byte(ui>>24)] + m_auiS[1][o_Byte(ui>>16)]) ^ m_auiS[2][o_Byte(ui>>8)]) + m_auiS[3][o_Byte(ui)];
}
...
inline std::string decrypt_file(std::string key, std::string file_name)
{
	...
	{
		//we load file data
		file.seekg(0, ios::end); // position get-ptr 0 o_Bytes from end
