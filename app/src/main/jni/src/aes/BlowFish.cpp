...
CBlowFish::CBlowFish(unsigned char* ucKey, size_t keysize, const SBlock& roChain) : m_oChain0(roChain), m_oChain(roChain)
{
	...
	//Check the Key - the key length should be between 1 and 56 o_Bytes
	if(keysize>56)
		keysize = 56;
	...
	for(i=0; i<18; i++)
	{
		...
		for(int n=4; n--; )
		{
			...
			if(iCount == keysize)
			{
				//All o_Bytes used, so recycle o_Bytes
				iCount = 0;
				p = aucLocalKey;
			}
		}
...
//Semi-Portable o_Byte Shuffling
inline void o_BytesToBlock(unsigned char const* p, SBlock& b)
{
	unsigned int y;
	//Left
	b.m_uil = 0;
...
inline void BlockToo_Bytes(SBlock const& b, unsigned char* p)
{
	unsigned int y;
	//Right
	y = b.m_uir;
...
void CBlowFish::Encrypt(unsigned char* buf, size_t n, int iMode)
{
	...
	if(iMode == CBC) //CBC mode, using the Chain
	{
		...
		for(; n >= 8; n -= 8)
		{
			o_BytesToBlock(buf, work);
			work ^= chain;
			Encrypt(work);
			chain = work;
			BlockToo_Bytes(work, buf+=8);
		}
	}
...
void CBlowFish::Encrypt(const unsigned char* in, unsigned char* out, size_t n, int iMode)
{
	...
	if(iMode == CBC) //CBC mode, using the Chain
	{
		...
		for(; n >= 8; n -= 8, in += 8)
		{
			o_BytesToBlock(in, work);
			work ^= chain;
			Encrypt(work);
			chain = work;
			BlockToo_Bytes(work, out+=8);
		}
	}
	else if(iMode == CFB) //CFB mode, using the Chain
	{
