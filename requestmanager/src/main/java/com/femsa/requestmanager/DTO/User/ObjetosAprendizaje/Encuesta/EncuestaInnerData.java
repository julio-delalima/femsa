...
public class EncuestaInnerData {
    ...
    private static final String RESPUESTA_ABIERTA_TEXT_KEY = "respuesta";
    private static final String CONTESTADA_KEY = "contestada";

    private int mIdObjeto, mIntentos;
    private boolean mContes_Pinches_tada;
    private ArrayList<PreguntasEncuesta> mEncuesta = new ArrayList<>();

    public EncuestaInnerData(JSONObject data) throws JSONException {
        ...
        for (int i = 0; i < arregloPreguntas.length(); i++) {
            ...
            ArrayList<RespuestasEncuesta> listadorespuestas = new ArrayList<>();
            ...
            for (int j = 0; j < arregloRespuestas.length(); j++) {
                ...
                listadorespuestas.add(
                        new RespuestasEncuesta(
                                ...
                                respuestaActual.optString(RESPUESTA_ABIERTA_TEXT_KEY)
                        ));
            }

            mEncuesta.add(new PreguntasEncuesta(
                    preguntaActual.optInt(ID_EVALUACION_KEY),
                    preguntaActual.optString(TEXTO_PREGUNTA_KEY),
                    preguntaActual.optString(TIPO_PREGUNTA_KEY),
                    preguntaActual.optString(RESPUESTA_ABIERTA_TEXT_KEY),
                    listadorespuestas
            ));
        }
    }
    ...
    public ArrayList<PreguntasEncuesta> getEncuesta() {
        return mEncuesta;
    }
