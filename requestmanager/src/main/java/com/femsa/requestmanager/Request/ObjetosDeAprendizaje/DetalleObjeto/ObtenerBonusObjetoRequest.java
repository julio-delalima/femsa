...
public class ObtenerBonusObjetoRequest implements Callback<ResponseBody>{

    private static final String OBJECT_KEY = "idObjeto";


    private Context mContext;
    private ObtenerBonusObjetoResponseContract mListener;

    public interface ObtenerBonusObjetoResponseContract extends ResponseContract
    {
    ...
    public ObtenerBonusObjetoRequest(Context context) {
        mContext = context;
    }

    public void makeRequest(int idObjeto, String token, ObtenerBonusObjetoResponseContract listener) {
        mListener = listener;
        if (Utilities.getConnectivityStatus(mContext))
        {
            RequestManager.PostMethods postMethod = RequestManager.getCliente().create(RequestManager.PostMethods.class);
            Call<ResponseBody> request = postMethod.requestObtenerBonusObjeto(crearParametros(idObjeto), token);
