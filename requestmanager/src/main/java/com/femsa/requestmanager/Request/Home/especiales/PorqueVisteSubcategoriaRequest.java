...
public class PorqueVisteSubcategoriaRequest implements Callback<ResponseBody>, Parser.ParserListener<CuzYouSawResponse> {

    private Context mContext;

    private PorqueVisteSubcategoriaRequest.CuzYouSawRequestContract mListener;
    ...
    public void onResponse(Call<ResponseBody> call, retrofit2.Response<ResponseBody> response) {
        switch(response.code())
        {
            case RequestManager.CODIGO_SERVIDOR.OK:
                CuzYouSawParser parser = new CuzYouSawParser(response.code(), true);
    ...
    public PorqueVisteSubcategoriaRequest(Context context)
    {
        mContext = context;
    }

    public void makeRequest(String tokenUser, int idsubcategoria, PorqueVisteSubcategoriaRequest.CuzYouSawRequestContract listener)
    {
        mListener = listener;

        if (Utilities.getConnectivityStatus(mContext))
        {
            RequestManager.PostMethods requestCuzYourSaw = RequestManager.getCliente().create(RequestManager.PostMethods.class);
            Call<ResponseBody> request = requestCuzYourSaw.requestPorqueVisteSubCategoria(tokenUser, FilterSubcategoryProgramsRequest.crearParametros(idsubcategoria));
