...
public class TermsNoTokenRequest implements Callback<ResponseBody>, Parser.ParserListener<TermsAndConditionsResponseData> {
    ...
    public TermsNoTokenRequest(Context context)
    {
        mContext = context;
    }

    public void makeRequest(String language, TermsResponseContract listener)
    {
        mListener = listener;
        if (Utilities.getConnectivityStatus(mContext))
        {
            RequestManager.PostMethods postMethods = RequestManager.getCliente().create(RequestManager.PostMethods.class);
            Call<ResponseBody> request = postMethods.requestTermsNoToken(createParameters(language));
